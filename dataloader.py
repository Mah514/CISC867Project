...
class DriveDataset(Dataset):
    ...
    def __init__(self, pathImageDirectory, pathMaskDirectory, size=512):
        ...
        for i in files:
            image_path = os.path.join(pathImageDirectory, i)
            label_filename = i.split('_')[0] + '_manual1.png'
            label_path = os.path.join(pathMaskDirectory, label_filename)
            label_path = os.path.normpath(label_path)

            if not os.path.exists(label_path):
                #print(f"Label file not found, skipping image: {label_path}")
                continue
            ...
            # Ensure each label has 1 channel
            if label.ndim == 2:
        ...
        self.label = np.array(labels, dtype=np.float32) / 255.0
